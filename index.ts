...
export default function (pi: ExtensionAPI) {
  ...
  pi.on("before_agent_start", async (event, _ctx) => {
    ...
    const context = relevant.map(r =>
      `### ${basename(r.chunk.file)} (lines ${r.chunk.lineStart}-${r.chunk.lineEnd}, score ${r.hybrid.toFixed(2)})\n` +
    ...
    // more reliably than text buried at the top of a long system prompt.
      return {
        message: {
          customType: "rag",
          content:
            `[pi-local-rag] Retrieved ${relevant.length} chunk${relevant.length === 1 ? "" : "s"} from the local index by hybrid search (BM25 + vector) against the user's message above — the user did NOT write or ask for these.\n` +
            `They are best-effort recall hints and MAY be irrelevant. If a chunk doesn't clearly relate to the request, ignore it silently — do not mention, summarize, or act on off-topic hits. Cite a chunk only when you actually use it. Higher score = more relevant.\n\n` +
            context,
          display: false,
        },
      };
  ...
  pi.registerCommand("rag", {
    ...
    handler: async (args, ctx) => {
      ...
      // Auto-dismiss the status panel after 10s so it doesn't stay pinned.
      setTimeout(() => ctx.ui.setWidget("rag-status", undefined), 10_000);
    },
  });
  ...
  pi.registerTool({
    ...
    label: "RAG index",
    description:
      "Add files to the local pi-local-rag semantic index (SQLite FTS5 + local ONNX embeddings, fully offline). " +
      "USE THIS when you want future rag_query calls (or auto-injection) to recall a body of knowledge that isn't ordinary source code you'd grep — e.g. large docs, specs, PDFs/DOCX, design notes, an external reference corpus, or a folder you'll ask conceptual questions about later. " +
      "Chunks text (including PDF/DOCX with OCR fallback), embeds each chunk, and tracks the path so `/rag refresh` can pick up changes. " +
      "DON'T use this to 'open' or read a single file you already know the path of — use the read tool for that. Indexing is a one-time setup cost; querying is the payoff.",
    parameters: Type.Object({
      path: Type.String({ description: "File or directory to index (relative or absolute). Directories are walked recursively, honoring the extension allowlist and exclude patterns." }),
    }),
  ...
  pi.registerTool({
    ...
    label: "RAG query",
    description:
      "Semantic + keyword search over the local pi-local-rag index (hybrid BM25 + vector). " +
      "USE THIS for CONCEPTUAL recall across an already-indexed corpus — 'where do we handle auth token refresh?', 'find the section about rate limits', 'what does the spec say about retries' — especially when you don't know the exact wording or filename, or when the answer lives in docs/PDFs rather than code. Vector search finds paraphrases and synonyms that grep misses. " +
      "DON'T use this as a substitute for grep/ripgrep on an exact symbol or string, and DON'T use it to read a file whose path you already know (use the read tool). " +
      "Returns ranked chunks with file path, line range, and per-signal scores (bm25 / vector / hybrid); treat low hybrid scores (< ~0.3) as weak matches. Requires rag_index to have run first — check rag_status if results are empty.",
    parameters: Type.Object({
      query: Type.String({ description: "Natural-language question or topic. Phrase it as what you're looking for, not just keywords — the vector side rewards descriptive queries." }),
      limit: Type.Optional(Type.Number({ description: "Max chunks to return (default 10). Lower it (3–5) for a focused answer, raise it when surveying a topic." })),
    }),
    execute: async (_toolCallId, params) => {
      const index = loadIndex();
  ...
  pi.registerTool({
    ...
    label: "RAG status",
    description:
      "Report on the local pi-local-rag index: files indexed, chunk count, vector coverage (% of chunks embedded), embedding model, tracked paths, and current RAG config (auto-injection on/off, topK, score threshold, alpha). " +
      "USE THIS to sanity-check before relying on rag_query — e.g. when rag_query returns nothing (is anything indexed?), when results look stale, or when vector coverage is below 100% (embeddings failed or are mid-build). Read-only, no arguments.",
    parameters: Type.Object({}),
