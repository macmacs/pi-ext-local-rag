...
Only for current project folder:

```bash
pi install git:github.com/macmacs/pi-ext-local-rag -l
```
...
**Easy fix:** Disable the proxy etc, run `/rag refresh` in `pi` (this downloads the model), re-enable the proxy.
