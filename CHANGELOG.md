...
- **New default embedding model: `Xenova/multilingual-e5-small`** (was `Xenova/all-MiniLM-L6-v2`). Retrieval-trained and multilingual, still 384-dim so the `chunks_vec` schema is unchanged. Costs ~113 MB instead of ~22 MB and roughly 2x the embed time. **Existing indexes re-embed themselves on the next index run** - no action needed, but the first run after upgrading is a full rebuild. Pin the old model with `"embeddingModel": "Xenova/all-MiniLM-L6-v2"` to opt out.
- **The embedding model is configurable** via `embeddingModel`. Supporting pieces:
  - `embed()` / `embedBatch()` take a `query` | `passage` argument, and E5-family models automatically get the `query: ` / `passage: ` prefixes they were trained with - without them most of E5's advantage disappears.
  - Changing the model now invalidates the index: `indexFiles` compares the configured model against the `embedding_model` metadata and, on a mismatch, wipes and re-embeds instead of silently mixing vectors from two incompatible coordinate systems.
